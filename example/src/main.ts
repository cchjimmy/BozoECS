...
import { createQuery, query } from "../../src/query.ts";
import { World } from "../../src/world.ts";

function random(min: number, max: number): number {
  return Math.random() * (max - min) + min;
}
function update(world: World, dt: number = 0) {
...
// query
const renderQ = createQuery(Position, Circle);
const moveQ = createQuery(Position, Velocity);
const bounceQ = createQuery(Position, Velocity, Circle);
